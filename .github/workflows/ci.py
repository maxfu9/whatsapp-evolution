"""CI helper to run Frappe parallel tests with optional coverage."""
...
def get_bench_path():
    """Return bench root from this helper file path."""
    return Path(__file__).resolve().parents[4]


class CodeCoverage:
    """Context manager for optional per-app coverage collection."""
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        if self.with_coverage and self.coverage:
            self.coverage.stop()
            self.coverage.save()
...
if __name__ == "__main__":
    ...
    with CodeCoverage(with_coverage=True, app=app_name):
        # pylint: disable=import-outside-toplevel,import-error
        if use_orchestrator:
            from frappe.parallel_test_runner import ParallelTestWithOrchestrator

            ParallelTestWithOrchestrator(app_name, site=site)
        else:
